#!/usr/bin/env bash
# An example automated PHP deployment script, similar to what is used by DMS.
#
...
eb deploy "$EB_ENV" --timeout=60 --version="$version" --verbose | tee /dev/stdout | grep "Environment update completed successfully."
if [ $? != 0 ]
then
    echo "Deploying $version as a new version."
    eb deploy "$EB_ENV" --timeout=60 --label="$version" --verbose | tee /dev/stdout | grep "Environment update completed successfully."
fi
if [ $? == 0 ]
then
